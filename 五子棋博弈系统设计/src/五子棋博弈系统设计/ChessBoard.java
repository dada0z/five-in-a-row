...
public class ChessBoard extends JPanel implements MouseListener {
	...
	if (!isComputer || (isComputer && currentRole == "玩家")) {
	    // 将鼠标点击的坐标位置转换成网格索引
	    ...
	    // 如果x，y位置已经有棋子存在，不能下
	    // 落在棋盘外不能下
	    if (isGameOver || hasChess(xIndex, yIndex) || xIndex < 0
		    || xIndex >= ROWS || yIndex < 0 || yIndex >= COLS)
		return;
...
//	    }
	...
	if (!isGameOver && isComputer && currentRole == "计算机") {
	    machineDo();
	    currentRole="玩家";
...
//	}
	...
	Point piece = new Point(x, y, color);
	piece.setX(x);
	piece.setY(y);
	// System.out
	// .println(piece.getColor().equals(Color.black) ? "黑方：" : "白方：");
	chessList[chessCount++] = piece;
	...
	if (returnVal == JFileChooser.APPROVE_OPTION) {
		...
		if (file.exists()) {
			...
			for (int i = 0; i < chessCount; i++) {
			    ...
			    String color = chessList[i].getColor().equals(
				    Color.black) ? "黑棋" : "白棋";
			    String location = "(" + chessList[i].getX() + ","
				    + chessList[i].getY() + ")" + "\r\n";
